...
func TestHandleList_PrintsStashesWithAuthorAndTime(t *testing.T) {
	...
	assert.Contains(t, out, "8stash/one")
	assert.Contains(t, out, "2h ago")
	assert.Contains(t, out, "Author One")
	assert.Contains(t, out, "stash 8stash/one", "Output should contain the commit message")
	assert.Contains(t, out, "8stash/two")
	assert.Contains(t, out, "1h ago")
	assert.Contains(t, out, "Author Two")
	assert.Contains(t, out, "stash 8stash/two", "Output should contain the commit message")
}
...
func TestHandleList_SortsOutputByName(t *testing.T) {
	...
	assert.Contains(t, out, "stash "+config.BranchPrefix+"apple", "Output should contain commit message for apple")
	assert.Contains(t, out, "stash "+config.BranchPrefix+"zebra", "Output should contain commit message for zebra")
}
